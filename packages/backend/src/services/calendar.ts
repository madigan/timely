...
import { oauth2Client, createCalendarClient, getUserInfo } from '../auth/oauth.js';
import { getUserTokens, storeUserTokens, needsRefresh } from '../auth/tokens.js';
...
async function refreshTokenIfNeeded(userId: string): Promise<boolean> {
  ...
  if (needsRefresh(tokens.expiryDate) && tokens.refreshToken) {
    try {
