...
import { getCalendarEvents, getUserCalendars } from "./calendar.service.ts"

export const calendarRoutes = new Elysia()
  .derive(requireAuth("CALENDAR"))
  ...
  .get(
    "/calendars/:id/events",
    async ({ params, query, userId, set }): Promise<CalendarEvent[] | ApiResponse> => {
      try {
  ...
  .get(
    "/calendars/events",
    async ({ query, userId, set }): Promise<CalendarEvent[] | ApiResponse> => {
      try {
        const { timeMin, timeMax } = query as {
          timeMin?: string
          timeMax?: string
